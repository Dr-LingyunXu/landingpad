...
ros::Publisher camStream_pub;

/******************************************************************************
 * @author Alex Wulff
 *
 * @par Description:
 * Callback for camera consolidation.
 *
...
{
  // transmit out
/*  ROS_INFO("Camera heard: [%s]", msg->data.c_str());
  ...
  // Forward to Main API
  camStream_pub.publish(msg);//forward);
  ros::spinOnce();
...
int main(int argc, char **argv)
{
  ...
  // publisher
  camStream_pub = cam.advertise<sensor_msgs::Image>("CamStream", 1000);

  // subscribe
  ros::Subscriber sub = cam.subscribe("CamData", 1000, cameraCallback);
