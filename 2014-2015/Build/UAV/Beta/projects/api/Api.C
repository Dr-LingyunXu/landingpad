...
ros::Publisher forward_pub,receive_pub;
void remoteReceive(int port);

void * Create_Listener(void * param)
{
  remoteReceive(1337);
}
...
int main(int argc, char **argv)
{
  ...
  receive_pub = n.advertise<std_msgs::String>("chatter", 1000);

  pthread_t listener[2];
  pthread_create(&listener[0], NULL, Create_Listener, NULL);
...
*
* @par Description:
* This function sets this shell receive commands from a specified port.
*
* @param[in] port - int to listen on specified ip.
*
* @returns none.
* 
*****************************************************************************/
...
void remoteReceive(int port)
{
  ...
  if (bind(sockfd, (struct sockaddr *) &serv_addr,
      sizeof(serv_addr)) < 0 && port != 1337)
  {
    ROS_WARN("ERROR on listen binding");
  }
  ...
  while (1)
  {
    ...
    if (newsockfd < 0)
    {
      ROS_WARN("ERROR on listen accept");
    }
    ...
    std::stringstream ss;
    ss << "Caught " << buffer << " over the wifi.\n";

    received.data = ss.str();

    if (!forward_pub)
    {
      ROS_WARN("Error with chatter forward.  Reiniting.\n");
      ros::NodeHandle temp;
      ros::Publisher recieve_pub = temp.advertise
                                   <std_msgs::String>("chatter", 1000);
    }

    receive_pub.publish(received); 
  }
  return;
}
