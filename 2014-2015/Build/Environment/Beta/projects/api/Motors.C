...
ros::Publisher motorStream_pub;

/******************************************************************************
 * @author Alex Wulff
 *
 * @par Description:
 * Callback for motor control consolidation.
 *
...
void motorCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  // Forward to Main API
  motorStream_pub.publish(forward);
  ros::spinOnce();
}
 ...
 *
 * @par Description:
 * Setup for middleman that is the motors node.
 *
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle drive;

  // publisher
  motorStream_pub = drive.advertise<std_msgs::String>("MotorStream", 1000);

  // subscribe
  ros::Subscriber sub = drive.subscribe("MotorData", 1000, motorCallback);
