...
void slamCallback(const sensor_msgs::PointCloud2 msg)
{
  ...
  {
    array.data.push_back(PointCloudXYZ.points[i].x);
    array.data.push_back(PointCloudXYZ.points[i].y);
    array.data.push_back(PointCloudXYZ.points[i].z);
    /*ss << PointCloudXYZ.points[i].x << " " << PointCloudXYZ.points[i].y << " " << PointCloudXYZ.points[i].z << "\n";
...
int main(int argc, char **argv)
{
  ...
  // publisher
  SlamStream_pub = slam.advertise<std_msgs::Float32MultiArray>("SlamStream", 1);
