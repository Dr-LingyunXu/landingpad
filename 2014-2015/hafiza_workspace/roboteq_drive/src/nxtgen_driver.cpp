	...
	error_count( 0 )
{
	...
	nh_priv.param<std::string>( "port", port, "/dev/ttyACM0" );
...
void NxtGenDriver::jointTrajCallback( const trajectory_msgs::JointTrajectoryConstPtr &msg )
{
	...
	for( unsigned int i = 0; i < msg->joint_names.size( ); i++ )
	{
		...
		{
			if( operating_mode == OperatingModes::ClosedLoopPosition )
			{
				pos = encoder_ppr * msg->points[0].positions[i] / ( 2.0 * M_PI );

				result = dev.SetCommand( _GO, 1, ( int )( pos + 0.5 ) );
			}
			else
			{
				if( operating_mode == OperatingModes::OpenLoopSpeed )
					rpm = 1000 * msg->points[0].velocities[i];
				else if( operating_mode == OperatingModes::ClosedLoopSpeed )
				{
		...
		else if( msg->joint_names[i] == ch2_joint_name )
		{
			if( operating_mode == OperatingModes::ClosedLoopPosition )
			{
				...
				result = dev.SetCommand( _GO, 2, ( int )( pos + 0.5 ) );
			}
			else
			{
				if( operating_mode == OperatingModes::OpenLoopSpeed )
